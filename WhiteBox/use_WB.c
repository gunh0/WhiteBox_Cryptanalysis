...
void wbaes_tbox(unsigned char in[16], unsigned char out[16])
{
    memcpy(out, in, 16);
    ...
    for (size_t i = 0; i < 9; i++)
    {
        ...
        {   
            unsigned char x = tbox[i][j][out[j]];
            out[j] = x;
        }
    ...
    for (size_t j = 0; j < 16; j++)
    {
        unsigned char x = tbox[9][j][out[j]];
        out[j] = x;
    }
}

int main() {
    unsigned char plain[16] = "testtesttesttest";
    unsigned char out[16] = { 0 };
    
    wbaes_tbox(plain, out);
